...
def _validate_file_to_download(gnos_id, filename, md5, object_id):
    """ Validate a file to download to collaboratory
        Args:
            gnos_id (str):      An existing entity's id from GNOS
            filename (str):     A file path in the local system
            md5 (str):          The MD5 sumf of corresponding to the file
            object_id (str):    The object id related to the file to download
        Raises:
            ...
            ValueError: No entity has been created for the file to download
    """
    if filename == None:
...
def validate_manifest_file(gnos_id, manifest_file):
    """ Check if the manifest file has valid informations to download to Collaboratory
        Args:
            gnos_id (str):          An existing entity's id from GNOS
            manifest_file (str):    A manifest_file containing download informations
        Returns:
    ...
    with open(manifest_file, 'r') as f:
        ...
        for row in reader:
            try:
                _validate_file_to_download(gnos_id, row['file_name'],row['md5'],row['object_id'])
            except ValueError, err:
...
def quick_download(gnos_id, files, icgc_storage_client, force=True):
